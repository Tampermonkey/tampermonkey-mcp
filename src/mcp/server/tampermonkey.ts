...
function ensureConnected(wsServer: TampermonkeyWebSocketServer): void {
    if (!wsServer.isConnected) {
        throw new Error('Not connected to Tampermonkey Editors. Call tampermonkey_get-connection-code first to get the connection code, then enter it in the Tampermonkey Editors extension to connect.');
    }
}
...
export const init = async (mcpServer: McpServer) => {
    ...
    // Tool: tampermonkey_get-connection-code
    mcpServer.tool(
        'tampermonkey_get-connection-code',
        `
